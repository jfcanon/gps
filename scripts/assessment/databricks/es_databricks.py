"""ES checks for Azure Databricks (MCSB v3). Cluster VMs are customer-managed but not ARM-readable."""


def _u(n, c, f, u): return {"resource": n or "all", "control_id": c, "feature": f, "status": "UNKNOWN",
                              "actual_value": "Databricks cluster VMs are ephemeral and managed by the Databricks control plane. EDR agents can be installed via init scripts but are not ARM-readable per workspace.",
                              "expected_value": "Install EDR/antimalware via cluster init scripts if required", "evidence_url": u}
